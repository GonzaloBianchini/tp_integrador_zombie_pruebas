...
Game_play::Game_play(Player& player, int numZombie,sf::RenderWindow& window) : Sound_4(4), Sound_5(5), Sound_7(7)
{
    ...
    _textPlayer.setFillColor(sf::Color::Red);
    ...
    _textPuntaje.setFillColor(sf::Color::Red);
...
void Game_play::draw(sf::RenderWindow& window)
{
    ...
    ///Dibujo Plantas///
    for(Planta* p : _plant_manager._array_plantas)
    {
        window.draw(*p);
    }
    ...
    ///Dibujo Disparos///
    for(Disparo* disparo : _shoot_manager._array_disparos)
    {
        window.draw(*disparo);
    }
    ///Dibujo Plataformas///
...
void Game_play::updateShoot(sf::RenderTarget& window)
{
    ...
    for(auto it=_shoot_manager._array_disparos.begin(); it!=_shoot_manager._array_disparos.end();)
    {
        ...
        {
            if(Z1->isCollision(*disp)&& tipoDisparo != TIPO::BRAIN) ///pregunto si el disparo saca vida o no
            {
...
void Game_play::update(sf::RenderTarget& window)
{
    if(_game_over==false)
    {
        if(_estado==ESTADOS_GAME_PLAY::ACTION) //SE EJECUTA SI EL JUEGO NO ESTÁ EN PAUSA
        {
            ...
            else    /// Guardamos partida si nos quedamos sin vidas
            {
                vidas=Z1->getvida();
                ...
                ///WIN CONDITION, guardo partida tambien
            ...
            if(Z1->gettimeshoot() > 2.0)    ///tiempo de recarga de tiro
            {
...
void Game_play::updatePlants2()
{
    ...
    updatePlantDeletion();  ///aca se borran y/o actualizan
...
void Game_play::updatePlantGeneration()
{
    ...
    if(_plant_manager._array_plantas.size() < 4  &&  enemigos<=20)   //define la cantidad de plantas en pantalla y condicion de winner
    {
